...
- **2026-07-22**: Adicionado campo `dueDay` (dia de vencimento) em `Account`, distinto e reintroduzido ao lado de `closingDay` (dia de fechamento) — ambos coexistem agora na aba Contas (colunas e modais separados), só para `cardType === 'credito'`. Notificações in-app (painel persistente, `AppNotification`) disparadas quando a data atual bate `closingDay` ou `dueDay` de algum cartão: `checkClosingDayNotifications()` e `checkDueDayNotifications()` em `src/App.tsx`, chamadas a cada `updateKPIs()`. Dedupe por `accountId_monthKey` via `getClosingDayFired`/`getDueDayFired` (`src/storage/persistence.ts`, localStorage apenas — sem claim multi-dispositivo). Schema/backend atualizados (`due_day` em `database/schema.sql` e `server/src/index.ts`).
